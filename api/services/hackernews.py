...
async def search_hn(query: str, source_id: str, tags: str = "story", hours_back: int = 24, max_results: int = 50) -> int:
    since_ts = int((datetime.utcnow() - timedelta(hours=hours_back)).timestamp())
    params = {"query": query, "tags": tags, "numericFilters": f"created_at_i>{since_ts}", "hitsPerPage": max_results}
    ...
    async with AsyncSessionLocal() as db:
        for hit in hits:
            ...
            if not eid or await _is_duplicate(eid): continue
            db.add(FeedItem(**_build_feed_item(hit, source_id)))
            inserted += 1
...
async def fetch_all_configured_hn_sources() -> dict:
    results = {}
    async with AsyncSessionLocal() as db:
        result = await db.execute(select(Source).where(Source.type == SourceType.hackernews, Source.is_active == True))
