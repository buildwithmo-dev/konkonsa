...
class Source(Base):
    ...
    config = Column(JSON, default={})          # subreddits, keywords, etc.
...
class Classification(Base):
    ...
    item_type = Column(SAEnum(ItemType), default=ItemType.unknown)
...
class Trend(Base):
    ...
    category = Column(SAEnum(ItemType), default=ItemType.trend)
...
class Solution(Base):
    ...
    status = Column(SAEnum(SolutionStatus), default=SolutionStatus.draft)
    created_at = Column(DateTime, server_default=func.now())
    updated_at = Column(DateTime, server_default=func.now(), onupdate=func.now())
...
class Alert(Base):
    ...
    id = Column(String, primary_key=True, default=gen_uuid)
    name = Column(String, nullable=False)
    condition = Column(JSON, default={})        # {type: "keyword", value: "AI", threshold: 10}
...
class Job(Base):
    ...
    description = Column(Text, nullable=True)
    status = Column(SAEnum(JobStatus), default=JobStatus.idle)
