...
from models import (
    ...
    Job, JobLog, JobStatus, AppSettings
)
from schemas import (
...
async def summary_stats(db: AsyncSession = Depends(get_db)):
    ...
    total_src = (await db.execute(select(func.count()).select_from(__import__('models').Source))).scalar_one()
...
async def activity_heatmap(db: AsyncSession = Depends(get_db)):
    ...
    result = await db.execute(
        select(
            func.strftime("%w", FeedItem.fetched_at).label("dow"),
            func.strftime("%H", FeedItem.fetched_at).label("hour"),
...
async def version():
    return {"version": "1.0.0", "model": "claude-sonnet-4-20250514"}

# ─────────────────────────────────────────────
#  JOBS (Update this section)
# ─────────────────────────────────────────────
jobs_router = APIRouter(prefix="/jobs", tags=["Jobs"])

@jobs_router.post("/run_all", response_model=MessageResponse)
async def run_all_jobs(background_tasks: BackgroundTasks, db: AsyncSession = Depends(get_db), _user=Depends(get_current_user)):
    """Triggers all active scrapers and classification jobs immediately."""
